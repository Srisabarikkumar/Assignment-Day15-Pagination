...
title.appendChild(document.createTextNode("Pagination Task"));
document.body.appendChild(title);
const description = document.createElement("p");
description.setAttribute("id", "description");
description.appendChild(
  document.createTextNode(
    "This is a pagination demonstration with HTML table using javascript DOM"
  )
);
document.body.appendChild(description);
const content = document.createElement("div");
content.setAttribute("class", "table-responsive")
...
const personPerRow = 5;

const rowCount = data.length / personPerRow;

document.body.appendChild(content);

function tableDisplay(start, limit) {
  let slicedData = data.slice(start, limit);
...
for (let i = 1; i <= rowCount; i++) {
  ...
  content.style.color = "milkywhite";
  btn.innerText = i;
  let start = (i - 1) * personPerRow;
  console.log(start);
  let limit = personPerRow + start;
  btn.onclick = function () {
    tableDisplay(start, limit);
  };
